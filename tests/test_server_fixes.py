...
@pytest.mark.parametrize('profile,registered',[
    ...
    (SimpleNamespace(fio='Doctor',specialization=None,invite_link='https://t.me/+test-invite'),False),
...
def test_signed_get_preserves_completion_rules(state,profile,registered):
    ...
    if not registered:
        assert "fetch('/pediatrics/register'" in r.text
...
def test_registration_preserves_fields_and_operation_order(state):
    ...
    assert ('specialization' in params)==True
